...
class L2SGDReductor(BasicObject):
    """L2-optimal reductor based on stochastic gradient descent.
    ...
    def reduce(self, rom0, maxit=100, tol=1e-4, batch_size=10, alpha=1,
               seed=0):
        """Reduce using optimization.
        ...
        tol
            Tolerance for optimization.
        batch_size
            Number of parameter values to sample in each iteration.
        alpha
            Learning rate.
        seed
        ...
        for i in range(maxit):
            ...
            mus = self.parameter_space.sample_randomly(
                batch_size,
                seed=sq.spawn(1)[0].generate_state(1),
            )
            rom_old = self.roms[-1]
            rom_new = self._update(rom_old, mus, alpha)
            self.roms.append(rom_new)
            dist_rel = _rel_l2_dist(rom_old, rom_new, self.parameter_space)
            self.dist.append(dist_rel)
            self.logger.info(f'ROM change: {dist_rel:.3e}')
            error = ((self.fom - rom_new).l2_norm(self.parameter_space)
                     / self.fom.l2_norm(self.parameter_space))
            self.errors.append(error)
            self.logger.info(f'Rel. error: {error:.3e}')
            if dist_rel <= tol:
                break
        return self.roms[-1]

    def _update(self, rom, mus, alpha):
        """Compute gradient of the squared L2 error.
        ...
        grad = np.mean([func(mu) for mu in mus], axis=0)
        rom_new = _vec_to_rom(_rom_to_vec(rom) - alpha * grad, rom)
        return rom_new
